...
let msgFn = function () {
    ...
    move();
    function inputWord() {//让字体一个一个的蹦出来
        let str = $lis[n].innerText;//要显示的全部内容
        let str2 = '';//当前要显示字体的内容
        let timer2 = null;
        let index = 0;
        timer2 = setInterval(function () {
            if(index == str.length){//说明字已经全部显示完毕
                ...
                $btn.tap(function () {
                    ...
                    move();
                });// 点击按钮要做  1 清空textBox  2 让键盘下去  3 对话框接着弹出
                return;
            }
            str2 += str[index];
            $textBox.html(str2);
